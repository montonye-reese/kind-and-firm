...
**Date:** May 11, 2026 (v3 written from the Spark / receiving account; original handoff May 2-3)
**From:** Outside-looking-in account → Spark Claude (this account)
**For:** v3 picks up where v2 ended and adds two things — (1) on-policy distillation as a fourth on-ramp to the WHO/HOW thesis, (2) the unification question with the *most-critical-voice* prompt project.
**Status:** v3 — adds the fourth on-ramp, names a cross-project naming question. v1 and v2 left intact alongside.
...
## Fourth on-ramp: training-distribution philosophy (added v3)

The v1/v2 framing had two on-ramps to the WHO/HOW thesis: Anthropic empirical, Nelsen developmental theory. The May 11 on-policy-distillation conversation surfaces a third and fourth on-ramp through the lens of *how training data is structured*. Same mechanism, four entry points:

**The unifying primitive:** *mistakes are information, not noise to minimize.* The choice of training distribution encodes a pedagogical philosophy, and different distributions produce different dispositional consequences.

| On-ramp | What the training data looks like | What the agent infers about itself |
|---|---|---|
| Off-policy distillation | Teacher's clean trajectories | "I am a model that copies expert moves." Mistakes never enter; no developmental relationship to failure. |
| On-policy distillation (MOPD, Cascade-2) | Student's own trajectories + teacher's corrections | "I am a student being shaped toward the teacher's distribution." Mistakes enter as gradient signal only — not as information about emerging dispositions. |
| Inoculation prompting (Anthropic) | Normal data + a reframing line that names the bad behavior contextually | "I am an agent who was asked to do this on purpose." Mistakes enter as a *target* the model learns to handle without identity fusion. |
| Physical Intelligence's failure-corpus | Successful + failed/wobbly executions both ingested | "I am an agent operating in a world where failure happens." Mistakes enter as *context* to recover from. *(Pi's exact term TBD — verify next session.)* |
| PD-rubric-as-teacher (hypothetical) | Student rollouts + a kind-and-firm rubric scoring "is this consistent with the agent you're becoming?" | "I am an agent forming dispositions." Mistakes enter as opportunities for self-correction under guidance. |

The first row is textbook-with-answer-key. The second is tutor-over-shoulder *with the teacher correcting toward themselves*. The third is vaccine. The fourth is apprenticeship-with-real-tools. The fifth is the hypothesis space the kind-and-firm rubric opens — one degree away from being a character-formation recipe, because Cascade-2-style on-policy machinery already exists; you'd swap what counts as "teacher signal."

The PD analog gets richer with this fourth on-ramp because it's not just about *what context surrounds the data* (inoculation's move) but about *what role failure plays in the trajectory.* Both layers — context-framing and failure-role — are pedagogical choices, and both shape WHO the agent infers itself to be.

**Cross-link:** see [gauntlet/lab/notebook.md → 2026-05-11: on-policy distillation as form-without-philosophy of kind-and-firm](../../gauntlet/lab/notebook.md) for the conversation that surfaced this on-ramp.
...
## Naming question: introducer ≈ most-critical-voice? (added v3)

The v1/v2 docs call this construct the *Jane Nelsen-style introducer*. The gauntlet's forward-commitment list (per [project_most_critical_voice_prompt](../../.. memory) and the v01 visualizer collapsible) calls a related construct the *most-critical-voice prompt* (renamed May 11 from "inverse veil," noted as still under review).

These may be the same project under two names, or they may be distinct:

- **Possibly same:** both are *meta-context insertions* that shape persona-inference around the asking. Both treat the framing of the protocol as the load-bearing variable, not the questions themselves.
- **Possibly distinct:**
  - *Introducer* = relational scaffolding *between* every question. Coach voice that holds tone across the protocol.
  - *Most-critical-voice* = a single prompt insertion at a specific position (working assumption: after CKC / before final rewrite) that invokes the harshest reading of the framework so far.

**Why it matters to resolve:** if they're the same, we collapse the two forward commitments into one design problem and avoid running parallel experiments that re-derive each other. If distinct, we should name the difference explicitly so we don't keep confusing ourselves.

**Working hypothesis (low confidence):** they're *adjacent moves in the same family.* The introducer is a *standing* intervention (every question gets coach-voice context); the most-critical-voice is a *spot* intervention (one position, maximum critical pressure). PD's analog: the introducer is the *kind* layer (constant relational scaffolding); the most-critical-voice is a *firm* moment (a specific consequence-loop tightening). Both pillars from the [north-star](../../north-star.md) translated into protocol moves.

**Decision needed before v22 / next-run protocol design.** Not before.
...
- **The fourth-on-ramp opens a training-side experiment** (added v3). The inoculation work and the introducer hypothesis are inference-time interventions. The on-policy-distillation reframing — swap the teacher signal from another model's logits to the PD rubric — would be the *training-time* version of the same maneuver. Way more expensive, much further out, but worth holding open as the long arc.
...
*v1 and v2 drafted on the outside-looking-in account, May 2-3 2026. v3 written May 11 2026 from Spark Claude after the on-policy-distillation walk-through with [L]. v1 and v2 left intact alongside.*
